...
3. Cоздать декоратор для замера времени выполнения функции
"""
...
def run_timer(func):  # Способ без functools
...
def run_timer_wraps_mode(func):  # С использованием functools
    @wraps(func)
    def wrapper(*args, **kwargs):
        start_time = time()
        func(*args, **kwargs)
...
#@trace
def fibonacci(position):  # Каждое последующее число равно сумме двух предыдущих чисел последовательности Фибоначчи
    print("--> fib(%d)" % position)
    result = 1
    if position > 1:
        result = fibonacci(position - 1) + fibonacci(position - 2)
    print("<-- fib(%d) == %d" % (position, result))
    return result
